...
func updateKey(w http.ResponseWriter, r *http.Request) {
	uName := r.URL.Query().Get(":name")
	keyID, err := strconv.Atoi(r.URL.Query().Get(":keyname"))
	defer r.Body.Close()
	...
	key := user.Key{ID: keyID, Text: string(content)}
	...
	fmt.Fprintf(w, "Key %q successfully updated!", keyID)
}

func removeKey(w http.ResponseWriter, r *http.Request) {
	uName := r.URL.Query().Get(":name")
	keyID, _ := strconv.Atoi(r.URL.Query().Get(":keyname"))
	if err := user.RemoveKey(uName, keyID); err != nil {
		http.Error(w, err.Error(), http.StatusNotFound)
		return
	}
	fmt.Fprintf(w, "Key \"%s\" successfully removed", keyID)
}
